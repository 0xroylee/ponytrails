	...
	return (
		...
		>
			<div className="flex items-center gap-2 text-muted-foreground">
				<ListChecks size={14} />
				<Typography variant="eyebrow">{phaseLabel} checkpoints</Typography>
			</div>
			<div className="grid gap-1">
				{checkpoints.length > 0 ? (
					...
					<Typography className="text-muted-foreground" variant="mono">
						No checkpoints recorded for this stage.
					</Typography>
				)}
			</div>
		</section>
...
function CheckpointRow({
	...
	return (
		<div
			className="flex min-h-8 min-w-0 items-center gap-2 overflow-hidden rounded-sm px-1.5 py-1"
