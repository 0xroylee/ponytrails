...
export function TaskCreateLogPanel({
	...
	return (
		<div className="grid max-h-52 gap-1 overflow-auto rounded-lg border border-theme-default bg-theme-control-subtle p-3 font-mono text-xs text-theme-secondary">
			{logs.map((line) => (
				<p
					className={cn(
						"m-0 whitespace-pre-wrap break-words",
						line.stream === "stderr" && "text-amber-200",
						line.stream === "system" && "text-theme-muted",
					)}
					key={line.id}
				>
					{line.text}
